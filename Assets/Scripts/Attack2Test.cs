...
public class Attack2Test : MonoBehaviour
{
    public PositionTester Player1;
    public int StartingRound;
    bool ShieldRoundCheck;
   ...
   void Attack2()
   {
        if(PlayerType == 1 && Player1._Round == true)
        {
            // Shield
            GetComponent<LifeManager>().OnShield = true;
            StartingRound = GameObject.Find("GameManager").GetComponent<GameManager>().RoundCount;
            ShieldRoundCheck = true;
        }
        else if(PlayerType == 2 && Player1._Round == true && GetComponent<LifeManager>().Life < 3)
        {
            // Life++
