...
public class WallTransparency : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == "Player" || other.gameObject.tag == "EnergyUp")
        {
            foreach(GameObject _object in GetComponentInParent<Wall>().Models)
            {
                ...
                _object.GetComponent<MeshRenderer>().material = _object.GetComponent<MaterialsData>().TransparencyMat;
            }
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.gameObject.tag == "Player" || other.gameObject.tag == "EnergyUp")
        {
            foreach (GameObject _object in GetComponentInParent<Wall>().Models)
            {
                _object.GetComponent<MeshRenderer>().material = _object.GetComponent<MaterialsData>().Material;
            }
        }
    }
}
